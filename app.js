...
listOfOrders.forEach((transaction) => {
  transactionHistoryContainer.innerHTML +=
    createTransactionHistoryItem(transaction);
});
...
function displayNumbers() {
  ...
  expenseDisplay.textContent =
    totalExpense === 0 ? "$0" : formatter(totalExpense);
  if (totalBudget < 0) {
    budgetDisplay.textContent = `-$${totalBudget * -1}`;
  } else {
    budgetDisplay.textContent =
      totalBudget === 0 ? "$0" : formatter(totalBudget);
  }
}
...
function getIconUrl(selectedCategory) {
  switch (selectedCategory) {
    case "Salary":
      return "/income-icon.png";
    case "Housing":
      return "/housing-icon.png";
    case "Transportation":
      return "/transport-icon.png";
    case "Food":
      return "/food-icon.png";
    case "Bills":
      return "/bill-icon.png";
    case "Others":
...
orderSubmitButton.addEventListener("click", (e) => {
  ...
  transactionHistoryContainer.innerHTML +=
    createTransactionHistoryItem(newObject);
...
resetHistoryButton.addEventListener("click", (e) => {
  e.preventDefault();
  let resetConfirm = window.confirm(
    "Are you sure, you will lose all previous history"
  );
  if (resetConfirm) {
    localStorage.clear();
